import threading
import psycopg2
# import mysql.connector # it is located in tfunction.py file.
import tfunction
...
def handle_data():
    try:
        ...
        last_count = tfunction.read_count_files(f'{addup_path}')
        # print(last_count)
        
        # PostgreSQL -------------------------------------------------------
        # get the previous counter
        sck_idx_last, sck_idx_dt = tfunction.get_most_recent_file_and_content(pg_path)
                ...
                # integer ==================================== 카운터, 측정값 등..
                    ...
                    if int(v):
                        ...
                        if data_type=='count':
                            ...
                            # print(tfunction.read_file(f'{path0}/count'))
                            old_count = int(tfunction.read_file(f'{path0}/count'))
                            now_count = int(v)
                            ...
                            if count>0: #----------------------------------
                                ...
                                if bom_idxs_string:
                                    ...
                                    if result is not None:
                                        ...
                                        di['count'] = count
                                        di['sck_dt'] = sck_dt
                                        pri_idx = tfunction.production_count(di)
                                        del di
        ...
        # make folder for variable of the previous count
        # sck_idx = '101123'
        tfunction.delete_all_files(pg_path)  # reset prev one
        if sck_idx:
