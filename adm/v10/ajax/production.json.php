...
if ($aj == "c1") {
	...
	if($pri_idx) {
		$pri = get_table('production_item','pri_idx',$pri_idx);

		$ar['table']  = 'g5_1_production_item';
		$ar['prd_idx'] = $pri['prd_idx'];
		$ar['bom_idx'] = $pri['bom_idx'];
		$ar['mms_idx']  = $mms_idx;
		$ar['mb_id']  = '';
		...
		$response->pri_idx = $arr['pri_idx'];
		$response->msg = "정보 등록 성공";
	}
	else {
		$response->err_code='E00';
		$response->msg = "정보 등록 실패!!";
	}
}
// production_member 삭제
if ($aj == "d1") {

	if($pri_idx) {
		$sql = " DELETE FROM {$g5['production_item_table']} WHERE pri_idx = '".$pri_idx."' ";
		sql_query($sql,1);
