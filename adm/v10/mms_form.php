...
<div class="tbl_frm01 tbl_wrap">
	...
	<tr>
		...
		<th scope="row">설비생산제품품질확인</th>
		<td>
			<?php echo help("장시간 휴지에 들어가는 설비에 '체크'해 주세요.<br>설비 최초 가동시 담당자에게 문자발송됨"); ?>
			<label for="mms_pos_yn">
               <input type="checkbox" name="mms_item_check_yn" id="mms_item_check_yn" value="1" <?=($mms['mms_item_check_yn']=='1')?'checked':''?> class="frm_input">
               설비의 생산되는 제품 품질확인 필요
            </label>
		</td>
	</tr>
...
function chk_Serial(object){
	...
	if(hx.test(str)){
		...
		$.ajax({
			...
			url : mms_chk_url,
			dataType : 'text',
			data : {'mms_idx' : mms_idx,'mms_serial_no' : st},
