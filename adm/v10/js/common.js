...
$(function(e) {
    ...
    // 관리자단 게시판관리 버튼 설정
    if(mb_level>4) {
        $('.btn_bo_user').prepend('<li><a href="./config_form.php?bo_table='+g5_bo_table+'" class="btn_admin btn btadm"><i class="fa fa-gears fa-fw"></i>&nbsp;환경설정</a></li>'); // 환경설정버튼 추가
        
        // $('.btn_bo_user .btn_admin i').removeClass('fa-spin');  // 설정 버튼 빙글이 제거
        // console.log($('.btn_bo_user').find('li').find('.btn_admin').length);
        // 중복으로 같은 버튼이 2개 표시되는데, 일단 2번째 생성된 "환경설정"버튼 삭제
        if($('.btn_bo_user').find('li').eq(1).find('.btadm').length){
            $('.btn_bo_user').find('li').eq(1).remove();
        }
    }
