...
// cst_send_type
if($cst_send_type) {
	for($i=0;$i<sizeof($cst_send_type);$i++) {
		// echo $cst_send_type[$i].'<br>';
		$cst_send_type_arr[] = $cst_send_type[$i];
	}
	$cst_send_type = implode(",",$cst_send_type_arr);	
}
// echo $cst_send_type.'<br>';
// print_r2($_REQUEST);
// exit;
...
// 업체정보 추출
if ($w!='')
	$cst = sql_fetch(" SELECT * FROM {$g5['customer_table']} WHERE cst_idx = '$cst_idx' ");


//지번 분리작업.
$cst_zip1 = substr($_POST['cst_zip'], 0, 3);
$cst_zip2 = substr($_POST['cst_zip'], 3);

// 업체명 히스토리
if($cst['cst_name'] != $cst_name) {
...
$sql_common = "	cst_name = '".addslashes($_POST['cst_name'])."'
                ...
                , cst_tel = '{$_POST['cst_tel']}'
                , cst_fax = '{$_POST['cst_fax']}'
                , cst_email = '{$_POST['cst_email']}'
                , cst_president = '{$_POST['cst_president']}'
                , cst_biz_no = '{$_POST['cst_biz_no']}'
                , cst_biz_type1 = '{$_POST['cst_biz_type1']}'
                , cst_biz_type2 = '{$_POST['cst_biz_type2']}'
                , cst_zip1 = '$cst_zip1'
                , cst_zip2 = '$cst_zip2'
                , cst_addr1 = '{$_POST['cst_addr1']}'
                , cst_addr2 = '{$_POST['cst_addr2']}'
                , cst_addr3 = '{$_POST['cst_addr3']}'
                , cst_addr_jibeon = '{$_POST['cst_addr_jibeon']}'
                , cst_send_type = '".$cst_send_type."'
                , cst_memo = '{$_POST['cst_memo']}'
                , cst_status = '{$_POST['cst_status']}'
";
...
if ($w == '') {
	...
	$sql = " INSERT into {$g5['customer_table']} SET
				{$sql_common} 
                , cst_reg_dt = '".G5_TIME_YMDHIS."'
                , cst_update_dt = '".G5_TIME_YMDHIS."'
	";
    sql_query($sql,1);
	$cst_idx = sql_insert_id();
...
// $sql = " DELETE FROM {$g5['customer_saler_table']} WHERE cst_idx='{$cst['cst_idx']}' $del_where ";
// sql_query($sql,1);


// 파일 처리2 (파일 타입이 여러개면 일련번호 붙여서 확장해 주세요.) ----------------
$fle_type2 = "customer_data";
...
for($i=0;$i<@count($_FILES[$fle_type3.'_file']['name']);$i++) {
	...
	if (${$fle_type3.'_del'}[$i] == 1) {
		if($mb_id) {
			delete_jt_file(array("fle_db_table"=>"company", "fle_db_id"=>$cst_idx, "fle_type"=>$fle_type3, "fle_sort"=>$i, "fle_delete"=>1));
		}
		else {
			// fle_db_id를 던져서 바로 삭제할 수도 있고 $fle_db_table, $fle_db_id, $fle_token 를 던져서 삭제할 수도 있음
...
$r = sql_query(" desc {$g5['customer_table']} ");
...
// exit;
if($w == 'u') {
	goto_url('./customer_form.php?'.$qstr.'&w=u&cst_idx='.$cst_idx, false);
	// alert('거래처 정보를 수정하였습니다.','./customer_form.php?'.$qstr.'&amp;w=u&amp;cst_idx='.$cst_idx, false); // 이상하게도 이건 stx 변수를 안 넘겨주네요.
}
else {
	alert('거래처 정보를 수정하였습니다.','./customer_list.php?'.$qstr, false);
}
?>