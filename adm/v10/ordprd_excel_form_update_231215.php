...
for($i=0;$i<=sizeof($allData[0]);$i++) {
    ...
    $arr['cst_idx'] = trim($list[0]); // 고객사
    $arr['bct_idx'] = trim($list[2]);    // 차종
    $arr['bom_part_no'] = trim($list[4]);    // 품번
    $arr['prd_start_date'] = trim($list[6]);  // 생산시작일
    $arr['prd_done_date'] = trim($list[7]);  // 생산완료일
    $arr['count'] = preg_replace("/,/","",$list[8]);      // 발주수량
        ...
        && is_numeric($arr['count']) )
    {
        ...
        if(!preg_match('/^LX2/',$arr['bct_name'])) {continue;}
