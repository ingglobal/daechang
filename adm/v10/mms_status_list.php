...
$where[] = " mms_idx IN ( SELECT mms_idx FROM {$g5['mms_table']} WHERE mms_status NOT IN('trash','delete') AND com_idx = '".$_SESSION['ss_com_idx']."' ) ";
...
$items1 = array(
    "mst_name"=>array("항목이름",0,0,0)
    ,"mst_type"=>array("타입",0,0,0)
    ,"mms_idx"=>array("설비",0,0,0)
    ,"mst_status"=>array("상태",0,0,0)
    ,"mst_reg_dt"=>array("등록일시",0,0,1)
);
?>
<style>
...
<div class="local_desc01 local_desc" style="display:no ne;">
    <p>설비 관련 품질기준정보, 비가동기준정보를 관리합니다.</p>
    <p>본 기준 정보들은 불량품 정보를 입력하거나 비가동 정보를 입력할 때 선택 항목으로 표시됩니다.</p>
</div>
...
<div class="tbl_head01 tbl_wrap">
	...
	<tr class="success">
        ...
        <?php
        $skips = array('mst_start_date','mst_end_date');
        if(is_array($items1)) {
            foreach($items1 as $k1 => $v1) {
                if(in_array($k1,$skips)) {continue;}
        ...
        //    print_r2($items1);
            foreach($items1 as $k1 => $v1) {
                ...
                // 타입
                    ...
                    $list[$k1] = '<a href="?ser_mms_idx='.$ser_mms_idx.'&sfl=mst_type&stx='.$row[$k1].'">'.$row[$k1].'  <span class="font_size_8">'.$g5['set_mst_type_value'][$row[$k1]].'</span></a>';
                }
