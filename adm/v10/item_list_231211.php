...
$where = array();
//$where[] = " (1) ";   // 디폴트 검색조건
$where[] = " ".$pre."_status NOT IN ('delete', 'trash') AND itm_type IN ('".implode("','",$g5['set_itm_type_key'])."') ";
...
// 상태
if($ser_itm_status) {
    $where[] = " itm_status = '".$ser_itm_status."' ";
}
...
$sql = " SELECT *
            , itm.cst_idx_customer AS cst_idx_customer
            , itm.cst_idx_provider AS cst_idx_provider
		{$sql_common}
		{$sql_search}
        {$sql_order}
...
// status count
$sql = " SELECT COUNT(*) AS cnt {$sql_common} {$sql_search} AND itm_status = 'pending' ";
$row = sql_fetch($sql);
...
.td_itm_subject a, .td_mb_name a {text-decoration: underline;}
.td_itm_price {width:80px;}
</style>

<div class="local_ov01 local_ov">
    <?php echo $listall ?>
    <span class="btn_ov01"><span class="ov_txt">총건수 </span><span class="ov_num"> <?php echo number_format($total_count) ?>건</span></span>
    <a href="?sfl=itm_status&stx=pending<?=$qstr?>" class="btn_ov01"> <span class="ov_txt">대기 </span><span class="ov_num"><?=number_format($pending_count)?>건</span></a>
</div>
...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$sql = "SELECT * FROM {$g5['file_table']}
				WHERE fle_db_table = 'item' AND fle_db_id = '".$row['itm_idx']."'
    ...
    <tr class="<?=$bg?>" tr_id="<?=$row[$pre.'_idx']?>">
        ...
        <td class="td_itm_status"><!-- 상태 -->
			<select name="itm_status[<?php echo $i; ?>]" id="itm_status_<?=$row['itm_idx']?>" style="width:100px;">
				<option value="">상태선택</option>
				<?=$g5['set_itm_status_value_options']?>
			</select>
			<script>$("#itm_status_<?=$row['itm_idx']?>").val("<?=$row['itm_status']?>").attr("selected","selected");</script>
        </td>
        <td class="td_admin">
			<?=$s_mod?>
		</td>
