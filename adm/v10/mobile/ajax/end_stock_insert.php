...
if($mms_manual_yn){
    ...
    //완제품이면서 자식중에 대표제품이 없으면 재고털이를 해야 한다.
    //내가 대표제품이면 부모제품을 기준으로 재고털이를 해야한다.
    if(($bom_type == 'product' && $main_bom_exist != '1') || $bit_main_yn == '1'){
        // $bom_idx_chief = ($bom_idx_parent) ? $bom_idx_parent : $bom_idx;
        // 재고털이한다.
        for($i=0;$i<$pri_cnt;$i++){
            $ar['pri_idx'] = $pri_idx;
            $ar['pic_value'] = 1;
            $ar['sck_dt'] = G5_TIME_YMDHIS;
