...
if($w == 'u') {
    for ($i=0; $i<count($_POST['chk']); $i++) {
        // 실제 번호를 넘김
        $k = $_POST['chk'][$i];
		$com = sql_fetch(" SELECT * FROM {$g5['customer_table']} WHERE com_idx = '".$_POST['com_idx'][$k]."' ");
...
// 삭제할 때
    ...
    for ($i=0; $i<count($_POST['chk']); $i++)
    {
        ...
        if (!$mb['mb_id']) {
            ...
            // 레코드 삭제
			$sql = " UPDATE {$g5['customer_table']} SET com_status = 'trash' WHERE com_idx = '{$_POST['com_idx'][$k]}' ";
			sql_query($sql,1);
