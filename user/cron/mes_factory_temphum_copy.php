...
if(!$ymd) {
    ...
    $sql = " SELECT event_time, DATE_ADD(event_time , INTERVAL +1 DAY) AS ymd FROM {$table1} ORDER BY mms_idx DESC LIMIT 1 ";
    // echo $sql.'<br>';
    $dat = sql_fetch($sql,1);
    // print_r2($dat);
...
for ($i=$st_timestamp; $i<time(); $i+=86400) {
	...
	$sql = "INSERT INTO {$table1}(work_date, work_shift, event_time, temp_avg, temp_max,temp_min,hum_avg,hum_max,hum_min)
            SELECT '".$current[$i]."', work_shift, CONCAT('".$current[$i]."',' ',SUBSTRING(event_time,12)) AS event_time, temp_avg, temp_max,temp_min
