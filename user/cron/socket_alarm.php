...
// 부하가 높아서 서버가 다운되는 거 같아서 1분에 한번 실행으로 변경했다가... 이제는 pgsql로 저장을 해 보자.
// sudo vi /etc/crontab
...
// meta 데이터가 있으면 마지막 이후 1분까지만 (cron 1분 주기, 나중에 혹시 누락 일괄 처리할 때 디비 부하가 너무 높으면 안 됨, 1분씩 끊어서 입력할 것!!) ------
$sql = " SELECT * FROM g5_5_meta WHERE mta_db_table = 'pgsql/socket/alarm' AND mta_key = 'sck_idx_last' ";
$one = sql_fetch($sql,1);
// print_r2($one);
if($one['mta_idx']) {
    // print_r2($one);
    $sql_end_dt = date("Y-m-d H:i:s", strtotime($one['mta_reg_dt'])+64);   // 64 seconds from now (1분보다 조금 더 넉넉하게..)
    // echo $sql_end_dt.BR;
...
if(!$one['mta_idx']) {
    $sql_start_dt = date("Y-m-d H:i:s", G5_SERVER_TIME-64);   // 64 seconds ago
...
// 알람데이터 처리 (1분동안 alarm이 있었던 것들만 디비 저장)
// print_r2($alarm);
            ...
            foreach($v1 as $k2=>$v2) {
                ...
                foreach($v2 as $k3=>$v3) {
                    ...
                    foreach($v3 as $k4=>$v4) {
                        ...
                        if($ar['mms_idx']&&$ar['com_idx']) {
                            // 변수 재설정
                            $ar[$pre2.'_update_dt'] = G5_TIME_YMDHIS;
                            // 공통쿼리
                            $skips[] = $pre2.'_idx';	// 건너뛸 변수 배열
                            $skips[] = $pre2.'_reg_dt';
                            for($i=0;$i<sizeof($fields2);$i++) {
                                if(in_array($fields2[$i],$skips)) {continue;}
                                $ar['sql_commons'][] = " ".$fields2[$i]." = '".$ar[$fields2[$i]]."' ";
                            }
                            // 공통쿼리 생성
                            $sql_common = (is_array($ar['sql_commons'])) ? implode(",",$ar['sql_commons']) : '';
                            // 중복 조건은 함수마다 다르게 설정!!
                            // $sql = "SELECT * FROM {$table2} 
                            //         WHERE cod_idx = '{$ar['cod_idx']}' AND arm_reg_dt = '{$ar['arm_reg_dt']}'
                            // ";
                            // // echo $sql.'<br>';
                            // $row = sql_fetch($sql,1);
                            // if($row[$pre2."_idx"]) {
                            ...
                            // else {
                                $sql = "INSERT INTO {$table2} SET 
                                            {$sql_common} 
                                            , ".$pre2."_reg_dt = '".G5_TIME_YMDHIS."'
                                ";
                                sql_query($sql,1);
                                $row[$pre2."_idx"] = sql_insert_id();
                            // }
                            // echo $sql.BR;
                        }
                        unset($ar);
